 ...
 * Usage: node backend/testing/scripts/stitch-sync.mjs [generate|fetch|all]
...
const DOCS_DIR = join(ROOT, 'backend', 'docs');
const STITCH_URL = 'https://stitch.googleapis.com/mcp';
const PROJECT_ID = process.env.STITCH_PROJECT_ID ?? '17256885408366407754';
const OUT_DIR = join(DOCS_DIR, 'design', 'stitch-screens');
...
async function uploadDesignMd() {
  const md = readFileSync(join(DOCS_DIR, 'design', 'DESIGN.md'), 'utf8');
