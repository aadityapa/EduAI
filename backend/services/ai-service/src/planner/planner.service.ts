...
export class PlannerService {
  ...
  async generate(user: UserContext, dto: PlannerGenerateDto) {
    await this.costService.checkQuota(user.tenantId, user.sub, 600);
    ...
    await this.conversationService.recordQuotaUsage(user.tenantId, user.sub, 600);

    return { conversationId: conversation.id, plan, tokensUsed: 600 };
  }
