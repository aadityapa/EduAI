...
1. **`backend/shared/nest-common`** — `AllExceptionsFilter`, `configureNestApp()`, `buildThrottlerModule()` with optional Redis storage
2. **`backend/shared/ai`** — `CircuitBreaker` integrated in `AiRouter`
3. **`backend/shared/shared`** — `withTimeout()`, `DB_QUERY_TIMEOUT_MS`
