...
|-----------|----------|--------|
| Prometheus scrape config | `backend/infrastructure/monitoring/prometheus.yml` | ✅ All 5 services + apps |
| Alert rules | `backend/infrastructure/monitoring/alerting-rules.yml` | ✅ CPU, memory, error rate, AI latency |
| Grafana dashboards | `backend/infrastructure/monitoring/grafana/dashboards/` | ✅ Platform + AI service |
| OTel collector | `backend/infrastructure/monitoring/otel-collector-config.yaml` | ✅ Traces to backend |
| Sentry config | `backend/infrastructure/monitoring/sentry.env.example` | ✅ Stub; needs DSN in prod |
| K8s probes | `backend/infrastructure/kubernetes/*.yaml` | ✅ liveness + readiness |
